...
- [x] **Tekuwa** — LEAN page (cat 101, `HIDE_PRODUCT_GRID`), no grid. Header photo + intro + TWO link boxes (cutting → tekuwa.de/en/cutting-lengthing/, stripping → tekuwa.de/en/lengthing-and-stripping/) side-by-side + sourcing box. Brand already in brands.ts (badge auto-resolves). Cat lives under Cutting Machines but ALSO added to the **Stripping Machines** dropdown (does both). **DONE.** ⚠️ stripping-box copy "provide and ensures" is the customer's verbatim (awkward grammar) — flagged, offered to tidy.
- [ ] **Mecal, Metzner, Ramatech, Mav Prüftechnik** — for each: inventory → pick Tier A/B → header photo (user supplies) + ranges/curated products or link box + sourcing box.
