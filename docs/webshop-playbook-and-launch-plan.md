...
- [x] **Feintechnik Rittmeyer (be-ri)** — Tier A (cat 106, leaf-of-flat-hub under Stripping Machines). New brand `feintechnik-rittmeyer` (be-ri logo badge). Header photo (AM.ALL.ROUND render from their site) + rewritten intro + single "Wire stripping machines" link box (eyebrow "be-ri cable processing machines") + grid KEPT (4 machines) + cleaned "Brand" filter + sourcing box. Both link box & sourcing box → rittmeyer-beri.de/en/cable-processing/. Swedish product names translated. **LOCKED & DONE.**
