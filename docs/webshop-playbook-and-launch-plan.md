...
>
> **🗂️ Menu-derived brand-box landings (the "Connectors" template, in rollout).** Every clickable dropdown entry should render a consistent **brand-box landing** (one box per brand under it), NOT a raw product grid (a grid at hub level felt like a "leak"). Boxes are derived from the menu tree via `getMenuBrandHub(route)` in `src/data/navigation.ts` (+ each brand's logo from `brands.ts`), so they stay complete/correct automatically (single source of truth, like the footer). A **category** (Connectors, Cutting Machines…) → its brand boxes; a **section root** (Industrial Components, Production Equipment) → **brands grouped by category** (user's choice); **single-brand categories** (Heat Shrink→HongShang) still show one box (user's choice, adds a click). The product grid then lives **only on the actual brand/leaf page**. Rendering: `BrandBoxCard` + `brandHubBlock` in `CatalogueCategoryPage.tsx`, activated per-category by `BRAND_HUB_CATEGORY_IDS` (rolling out one at a time; **Connectors 43 is the reference**), which forces off `showProductBrowser`/`showVisualLinks`/`showGenericCategoryCards`. Per-box logo overrides: `BRAND_BOX_LOGO_OVERRIDES[catId][slug]`. Retires the fragile old boxes that came from hand-authored Magento `description` HTML (which was missing Z&F etc.). ⚠️ **Sealed-connectors (43) logo rule (customer, this page only):** the **TE** box uses the customer's fresh tile `/media/brand-logos/te-connectivity.jpg` (swap TE here only, nowhere else); the **Deutsch** box keeps its existing correct mark `…/category_images/Deutsch.jpg` (leave as is). NB a latent global bug: `brands.ts` gives BOTH Deutsch and TE the same `te-connectivity.svg` (a TE mark) — do not "fix" it globally without the user's go-ahead.
>
