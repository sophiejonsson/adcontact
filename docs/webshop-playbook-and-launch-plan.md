...
>
> **🔲 Brand-page link boxes — layout by count (locked).** A section's boxes render by `boxes.length`: **1 box → full-width Zoller & Fröhlich-style banner** (big image left `sm:w-72 lg:w-96`, text right, `rounded-2xl`); **2+ boxes → compact side-by-side Branson grid** (`w-28 sm:w-40` image, `sm:grid-cols-2`). All 2-box cards share `min-h-[176px]` + `justify-center` text so every 2-box page is the **same size** (Wezag baseline); heavier-copy pages like Branson grow a touch taller. Change that one `min-h` to resize all 2-box pages at once. Always picture-LEFT / text-RIGHT (image-on-top was rejected). Config: `CategoryLinkSection` = `{eyebrow, boxes}` in `CatalogueCategoryPage.tsx`.
>
...
- [x] **Mav Prüftechnik (Test equipment)** — LEAN 2-box page on the repurposed Test & Quality hub (cat 76, `CATEGORY_BRAND_OVERRIDES`→mav, `HIDE_PRODUCT_GRID`). Header photo (KMG display) + intro + two boxes (Manual force testers, Motorized force testers → mav-germany.de) + sourcing box → mav-germany.de/home.html. Menu section "Test equipment" (after Branson) + footer. Unrepresented Test & Quality sub-brands hidden. **DONE.**
- [ ] **Mecal, Metzner, Ramatech** — for each: inventory → pick Tier A/B → header photo (user supplies) + ranges/curated products or link box + sourcing box.
